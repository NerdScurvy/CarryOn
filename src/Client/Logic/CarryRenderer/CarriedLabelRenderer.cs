...
namespace CarryOn.Client.Logic.CarryRenderer
{
    internal sealed class CarriedLabelRenderer : IDisposable
    {
        ...
        internal void TryRender(CarriedBlock carried, float[] initialMatrix, float[] viewMat, IStandardShaderProgram prog, BlockPos lightPos)
        {
            ...
            if (string.IsNullOrWhiteSpace(text) && labelStack?.Collectible == null) return;
            ...
            Mat4f.Translate(modelMat, modelMat, labelTransform.Translation.X, labelTransform.Translation.Y, labelTransform.Translation.Z);
            Mat4f.Translate(modelMat, modelMat, labelTransform.Origin.X, labelTransform.Origin.Y, labelTransform.Origin.Z);
            Mat4f.RotateX(modelMat, modelMat, labelTransform.Rotation.X * GameMath.DEG2RAD);
            Mat4f.RotateZ(modelMat, modelMat, labelTransform.Rotation.Z * GameMath.DEG2RAD);
            Mat4f.RotateY(modelMat, modelMat, labelTransform.Rotation.Y * GameMath.DEG2RAD);
            Mat4f.Scale(modelMat, modelMat, labelTransform.ScaleXYZ.X, labelTransform.ScaleXYZ.Y, labelTransform.ScaleXYZ.Z);
            Mat4f.Translate(modelMat, modelMat, -labelTransform.Origin.X, -labelTransform.Origin.Y, -labelTransform.Origin.Z);
            ...
            if (label == null) return; // silent fail
